	...
	{package MySQLDriver;
		...
		##--> Begin Method :: Open
			sub Open() {
				...
				if(!defined($this->{MyConnection})) {
					$this->{MyConnection} = DBI->connect("dbi:mysql:".$this->{Schema}.":".$this->{Host},$this->{Username},$this->{Password})
						or die("WebLegs.MySQLDriver.Open(): Can't connect to mysql db: (". $DBI::errstr .").");
				}
			}
		##<-- End Method :: Open
		...
		##--> Begin Method :: ExecuteNonQuery
			sub ExecuteNonQuery() {
				...
				$StatementHandle->execute()
					or die("WebLegs.MySQLDriver.ExecuteNonQuery(): Error executing statement: (". $DBI::errstr .").");
			}
		...
		##--> Begin Method :: GetDataString
			sub GetDataString() {
				...
				my $StatementHandle = $this->{MyConnection}->prepare($this->{SQLCommand});
				$StatementHandle->execute()
					or die("WebLegs.MySQLDriver.GetDataString(): Error executing statement: (". $DBI::errstr .").");
		...
		##--> Begin Method :: GetLastInsertID
			sub GetLastInsertID() {
				...
				$StatementHandle->execute()
					or die("WebLegs.MySQLDriver.GetLastInsertID(): Error executing statement: (". $DBI::errstr .").");
		...
		##--> Begin Method :: GetFoundRows
			sub GetFoundRows() {
				...
				$StatementHandle->execute()
					or die("WebLegs.MySQLDriver.GetFoundRows(): Error executing statement: (". $DBI::errstr .").");
		...
		##--> Begin Method :: GetDataRow
			sub GetDataRow() {
				...
				$StatementHandle->execute()
					or die("WebLegs.MySQLDriver.GetDataRow(): Error executing statement: (". $DBI::errstr .").");
		...
		##--> Begin Method :: GetDataTable
			sub GetDataTable() {
				...
				$StatementHandle->execute()
					or die("WebLegs.MySQLDriver.GetDataTable(): Error executing statement: (". $DBI::errstr .").");
		...
		##--> Begin Method :: GetDataArray
			sub GetDataArray() {
				...
				$StatementHandle->execute()
					or die("WebLegs.MySQLDriver.GetDataArray(): Error executing statement: (". $DBI::errstr .").");
